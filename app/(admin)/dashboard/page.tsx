...
export default async function DashboardPage() {
  ...
  const [profileRes, ordersRes, pendingReqsRes, subscribersRes, productsRes, usersRes, settingsRes] =
    await Promise.all([
  ...
  const categoryData = REAL_CATS.map(cat => ({
    name: cat,
    value: prods.filter(p => p.category === cat).length || 0,
  })).filter(d => d.value > 0);
  ...
  const SWATCH: Record<string, { emoji: string; color: string }> = {
    bag:      { emoji: "👜", color: "#2a1f14" },
    top:      { emoji: "👚", color: "#1f1420" },
    hat:      { emoji: "🧢", color: "#14201a" },
    scarf:    { emoji: "🧣", color: "#1a1420" },
    bikini:   { emoji: "👙", color: "#201414" },
    vest:     { emoji: "🧥", color: "#141a20" },
    polo:     { emoji: "👕", color: "#1a2014" },
  };
  ...
  const topProducts: TopProduct[] = prods
    ...
    .slice(0, 5)
    .map(p => {
      const key = Object.keys(SWATCH).find(k => p.name?.toLowerCase().includes(k)) ?? "bag";
      return {
        name: p.name ?? "Product",
        swatch: SWATCH[key].emoji,
        swatchColor: SWATCH[key].color,
        unitsSold: soldMap[p.id] ?? 0,
        price: p.price ?? 0,
        stockPct: Math.min(Math.round(((p.stock_count ?? 0) / 50) * 100), 100),
      };
    });

  const totalCustomers = usersRes.count ?? 1;
  const uniqueBuyers   = orders.reduce((set, o) => {
    const name = (o.shipping_address as { full_name?: string })?.full_name;
    if (name) set.add(name);
    return set;
  }, new Set<string>()).size;
  const avgOrderValue  = nonCancelled.length
    ? Math.round(nonCancelled.reduce((s, o) => s + (o.total_amount ?? 0), 0) / nonCancelled.length)
    : 0;
  const audienceData = {
    repeatPct: totalCustomers > 0 ? Math.round((Math.min(uniqueBuyers, totalCustomers) / totalCustomers) * 100) : 0,
    avgOrderValue,
    abandonPct: 24,
  };
  ...
  orders.forEach(o => {
    ...
    if (!dayStats[day]) dayStats[day] = { orders: 0, revenue: 0 };
    dayStats[day].orders++;
    dayStats[day].revenue += o.total_amount ?? 0;
  });
  const bestDayEntry = Object.entries(dayStats).sort((a, b) => b[1].orders - a[1].orders)[0];
  const bestDay = bestDayEntry ? {
    day: DAY_NAMES[Number(bestDayEntry[0])],
    orders: bestDayEntry[1].orders,
    revenue: Math.round(bestDayEntry[1].revenue / bestDayEntry[1].orders),
  } : null;

  const customerOrderCount: Record<string, number> = {};
  orders.forEach(o => {
    const name = (o.shipping_address as { full_name?: string })?.full_name;
    if (name) customerOrderCount[name] = (customerOrderCount[name] ?? 0) + 1;
  });
  ...
  const pendingActions = [
    { label: "orders need confirmation", count: pendingOrdersCount,        href: "/orders",          color: "bg-amber-500/10 border-amber-500/20 text-amber-300 hover:bg-amber-500/20" },
    { label: "custom requests unquoted", count: pendingReqsRes.count ?? 0, href: "/custom-requests", color: "bg-orange-500/10 border-orange-500/20 text-orange-300 hover:bg-orange-500/20" },
    { label: "products out of stock",    count: outStockCount,              href: "/products",        color: "bg-red-500/10 border-red-500/20 text-red-300 hover:bg-red-500/20" },
  ];
  ...
  const STATS: StatDef[] = [
    { label: "This Month",   numericValue: thisMonthRev, rawValue: `₦${thisMonthRev.toLocaleString()}`, sub: revTrend !== undefined ? "vs last month" : "Current month", iconKey: "TrendingUp", href: "/analytics", color: "text-[#4ade80]",  bg: "bg-[#4ade80]/10 border-[#4ade80]/20",   trend: revTrend },
    { label: "Total Orders", numericValue: orders.length,                                                sub: `${pendingOrdersCount} pending`,                           iconKey: "ShoppingBag", href: "/orders",    color: "text-blue-400",   bg: "bg-blue-500/10 border-blue-500/20",      trend: ordTrend },
    { label: "Customers",    numericValue: usersRes.count ?? 0,                                          sub: "Registered",                                              iconKey: "Users",       href: "/users",     color: "text-purple-400", bg: "bg-purple-500/10 border-purple-500/20" },
    { label: "Products",     numericValue: prods.length,                                                 sub: `${lowStockCount} low stock`,                              iconKey: "Package",     href: "/products",  color: "text-amber-400",  bg: "bg-amber-500/10 border-amber-500/20" },
    { label: "Requests",     numericValue: pendingReqsRes.count ?? 0,                                    sub: "Pending quote",                                           iconKey: "Scissors",    href: "/custom-requests", color: "text-orange-400", bg: "bg-orange-500/10 border-orange-500/20" },
    { label: "Subscribers",  numericValue: subscribersRes.count ?? 0,                                    sub: "Email list",                                              iconKey: "Mail",        href: "/subscribers", color: "text-pink-400", bg: "bg-pink-500/10 border-pink-500/20" },
  ];
  ...
  return (
    <div className="xl:flex xl:gap-6 xl:items-start space-y-5 xl:space-y-0">
      <div className="flex-1 min-w-0 space-y-5">
        <FadeSection delay={0}>
          <p className="text-[10px] tracking-[0.25em] uppercase text-[#6b9e7e] font-body mb-1">Overview</p>
        ...
        <FadeSection delay={200}>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="lg:col-span-2 bg-white/[0.03] backdrop-blur-sm border border-white/8 rounded-2xl p-5 chart-card min-w-0">
              <p className="text-[10px] tracking-[0.2em] uppercase text-[#4ade80]/60 font-body mb-0.5">Revenue</p>
              <p className="font-heading text-xl font-semibold text-white mb-5">Last 6 Months</p>
              <RevenueChart data={revenueData} />
            </div>
            <div className="bg-white/[0.03] backdrop-blur-sm border border-white/8 rounded-2xl p-5 chart-card min-w-0">
              <p className="text-[10px] tracking-[0.2em] uppercase text-[#4ade80]/60 font-body mb-0.5">Sales</p>
        ...
        <FadeSection delay={300}>
          <div className="bg-white/[0.03] backdrop-blur-sm border border-white/8 rounded-2xl p-5 chart-card min-w-0">
            <p className="text-[10px] tracking-[0.2em] uppercase text-[#4ade80]/60 font-body mb-0.5">Volume</p>
      ...
      <aside className="hidden xl:flex flex-col gap-4 w-[240px] shrink-0">
        <div className="bg-white/[0.03] backdrop-blur-sm border border-white/8 rounded-2xl p-5 flex flex-col gap-6 sticky top-6">
      ...
      <div className="xl:hidden space-y-4">
        <div className="bg-white/[0.03] backdrop-blur-sm border border-white/8 rounded-2xl p-5"><TopProducts products={topProducts} /></div>
        <div className="bg-white/[0.03] backdrop-blur-sm border border-white/8 rounded-2xl p-5"><AudienceMetrics data={audienceData} /></div>
        <div className="bg-white/[0.03] backdrop-blur-sm border border-white/8 rounded-2xl p-5"><QuickActions /></div>
      </div>
    </div>
  );
}
